...
def loso_cv(config_path, dataset_path=None):
    ...
    if config.SKIP_FINISHED_ARGS:
        ...
        if os.path.exists(folds_path):
            for fold_path in os.listdir(folds_path):
                existing_folds.append(fold_path)
    ##################################

    print('Perform an LOO')
    grid_cms = []
    grid_args = []
    ...
    for i, args in enumerate(src.utils.grid_search(hyperparameters)):
        ...
        cv_cms = []
        for j, train, test in src.utils.cv_split(subjects, 0,
        ...
        #                                                 config.GS_NUM_VALID,
        #                                                 config.CV_RANDOM):
            print(f'Fold {j}: valid={test} test={test}')
            if config.SKIP_FINISHED_ARGS and \
               'fold_'+str(j) in existing_folds:
                print('Skipping existing fold: ', 'fold_'+str(j))
                continue
            ...
            # Creating the image generator objects:
            # Train subset
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Start DL LOSO.')
